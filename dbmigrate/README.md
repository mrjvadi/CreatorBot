...
همان ۱۱ سرویس Postgres دار پلتفرم (رجوع `internal/migrate/registry.go`):
...
| archive-bot | archive_bot | pg_trgm و ایندکس GIN جستجو داخل baseline است |
| source-service | source_svc | |

سرویس‌های Mongo (uploader-bot, fraud-engine, log-collector, admanager-bot)
این‌جا نیستند — schema ندارند. agentmanager و webhook-gateway دیتابیس ندارند.
