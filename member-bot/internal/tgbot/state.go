...
const (
	...
	// ثبت قفل جدید
	stepLockChannel  step = "lock:channel"   // ارسال forward از کانال
	stepLockDuration step = "lock:duration"  // مدت (روز)
	stepLockPrice    step = "lock:price"     // قیمت روزانه

	// افزودن check bot
	stepAddBot step = "addbot:token"
...
func stateKey(uid int64) string { return fmt.Sprintf("mb:s:%d", uid) }
