...
3. نقشه‌ی کامل moduleها و processها
...
(۲۰۲۶-۰۷-۱۷) چهار ربات محصول (`uploader-bot`, `vpn-bot`, `archive-bot`,
`member-bot`) دیگر **هیچ Postgres ندارند** — همه‌ی داده‌شان روی MongoDB است.
`vpn-bot`/`archive-bot`/`member-bot` تا این تاریخ Postgres داشتند — تاریخچه‌ی
baseline در `dbmigrate/migrations/{vpn-bot,archive-bot,member-bot}/RETIRED.md`.
`archive-bot` جستجوی فازیِ خودش (قبلاً `pg_trgm`) را با trigram/Jaccard اپ‌ساید
(app-side) در Go بازسازی کرد (رجوع `archive-bot/internal/search`) — MongoDB
self-hosted معادل native ندارد.

**مدلِ tenancy (تصحیح‌شده در همان روز):** اول تصور شد هر instance دیتابیسِ Mongo
اختصاصیِ خودش را دارد — ولی این هرگز واقعاً پیاده نشد (نه در نسخه‌ی Postgres،
نه در migration اول به Mongo). واقعیت این است: **یک دیتابیسِ Mongo مشترک
به‌ازای هر نوع ربات** (`MONGO_DB=vpn_bot`/`archive_bot`/`member_bot`، ثابت
برای همه‌ی instanceهای آن نوع — برای بهترین عملکرد، بدونِ overhead مدیریتِ
دیتابیسِ جدا به‌ازای هر instance)، با **جداسازیِ منطقیِ instanceها با فیلدِ
`instance_id`** روی هر سند/کوئری — دقیقاً همان الگویی که `uploader-bot` با
`shared-core/docstore` (`Base.baseFilter()`) از قبل درست پیاده می‌کرد.
`instanceID` نیازی به env جدید ندارد؛ از `BOT_TOKEN` مشتق می‌شود
(`"bot_" + botID`، همان چیزی که `webhook.BotIDFromToken` از قبل محاسبه
می‌کرد). ایندکس‌های یکتا همه compound شدند (`instance_id` کلیدِ پیشرو). این
تصحیح دو باگِ واقعیِ cross-tenant را هم کشف/رفع کرد: `archive-bot.Search`
بدونِ `instance_id` فایل‌های یک مشتریِ دیگر را برمی‌گرداند (نشتِ دیتا)، و
`member-bot.ClearBotMemberships` بدونِ فیلتر memberships همه‌ی instanceهای
دیگر را هم پاک می‌کرد. جزئیاتِ کامل در CHANGELOG.md.

جداسازی فیزیکی کامل (سرور/instance جدا برای هرکدام، نه فقط دیتابیس جدا روی
همان سرور) هنوز انجام نشده — این قدم بعدی است اگر لازم شود (فقط برای سرویس‌های
مرکزیِ Postgres باقی‌مانده مطرح است). رجوع
...
## ۳. نقشه‌ی کامل moduleها و processها
...
| **apimanager** | کنترل‌پلین HTTP فعال برای user/admin | JWT، lifecycle، wallet/plan، admin CRUD و proxyهای محصول |
| **agentmanager** | روی هر سرور؛ Docker container کاربران را می‌سازد/مدیریت می‌کند | Docker SDK با DOCKER_HOST؛ معمولاً پشت docker-socket-proxy محدود |
...
| **dbmigrate** | CLI متمرکز migration ورژن‌دار برای هر ۸ سرویس Postgres دار (نه یک سرویس همیشه‌روشن) | baseline از AutoMigrate واقعی هر سرویس؛ `up`/`status`/`mark`/`new` — رجوع `dbmigrate/README.md` |
...
| **archive-bot** | آرشیو فایل با جستجوی فارسی fuzzy (trigram/Jaccard اپ‌ساید روی Mongo، رجوع بخش ۲) |
...
**vpn-bot** (MongoDB از ۲۰۲۶-۰۷-۱۷):
`User`, `Panel`, `Plan`, `Subscription`, `DiscountCode`, `Payment`
(`Setting` بلااستفاده بود، در migration حذف شد)

**member-bot** (MongoDB از ۲۰۲۶-۰۷-۱۷):
`Owner`, `Lock`, `CheckBot` (با `BotChannelMembership` embed‌شده به‌جای
join-table)، `Payment` (`MemberVerification`/`Setting` بلااستفاده بودند، حذف شدند)

**archive-bot** (MongoDB از ۲۰۲۶-۰۷-۱۷):
`User`, `Category`, `File` (با `Ngrams` برای جستجوی فازی؛ `Setting` بلااستفاده
بود، حذف شد)
...
| `membership.joined` | member-bot، **هر ربات رایگانِ متصل به کمپین** (uploader/vpn/archive-bot، رجوع `shared/pkg/joinevents`) | fraud-engine، community-service، **ads-bot** |
| `membership.left` | member-bot، ربات‌های رایگانِ بالا | fraud-engine |
| `freebot.created` | botmanager (وقتی instance رایگان ساخته شد) | ads-bot |
| `ads.bot_status_check` (request/reply) | ربات رایگان (startup + هر ۵ دقیقه) | ads-bot — «آیا الان به کمپینی وصل‌ام؟» (جایگزینِ کوئریِ Postgresِ حذف‌شده) |
| `ads.confirm_channel_admin` (request/reply) | ربات رایگان (وقتی خریدار آن را ادمینِ کانال کرد) | ads-bot — enforcement قفل شروع می‌شود |
...
۴. خریدار ربات‌ها را در کانال خودش ادمین می‌کند → همان ربات این را خودش
   تشخیص می‌دهد (`onMyChatMember` + چکِ دوره‌ایِ `ads.bot_status_check`) و
   به ads-bot خبر می‌دهد (`ads.confirm_channel_admin`) → قفل‌کردن شروع می‌شود
۵. کاربر واقعی عضو می‌شود → **خودِ همان ربات رایگان** (نه member-bot — رجوع
   بخش ۸، تصحیحِ ۲۰۲۶-۰۷-۱۹) تشخیص می‌دهد → membership.joined
۶. ads-bot می‌گیرد:
...
- ✅ (۲۰۲۶-۰۷-۱۷) قطع کامل دسترسیِ Postgres برای هر چهار ربات محصول
  (uploader-bot, vpn-bot, archive-bot, member-bot) — همه‌ی داده روی MongoDB.
  در همین migration دو باگ واقعیِ از‌پیش‌موجود هم رفع شد (race دوبار-اعتبار در
  vpn-bot `approvePayment`، و race ساختِ دستهٔ تکراری در archive-bot
  `FindOrCreateCategory`)، و جستجوی فازیِ archive-bot با trigram/Jaccard
  اپ‌ساید جایگزینِ `pg_trgm` شد.
- ✅ (۲۰۲۶-۰۷-۱۷، همان روز) رفعِ نبودِ جداسازیِ tenant در سه ربات بالا —
  `instance_id` روی هر سند/کوئری اضافه شد تا instanceهای مختلفِ یک نوع ربات
  که دیتابیسِ Mongo مشترک دارند دیتای هم را نبینند/رد نکنند (همان الگوی
  uploader-bot با `shared-core/docstore`). دو باگِ واقعیِ cross-tenant کشف
  و رفع شد: نشتِ نتایجِ جستجو بینِ مشتری‌های مختلفِ archive-bot، و پاک‌شدنِ
  memberships همه‌ی instanceهای member-bot با sync دوره‌ایِ یک instance
  (رجوع CHANGELOG.md برای جزئیاتِ کامل هر دو فاز).
- ✅ (۲۰۲۶-۰۷-۱۹) اتصالِ واقعیِ ربات‌های رایگان به ads-bot تکمیل شد — قبلاً
  هیچ ربات محصولی واقعاً `membership.joined` را برای کانالِ خریدار منتشر
  نمی‌کرد (member-bot هم چون فقط رباتِ خودش را رصد می‌کند، هرگز این کانال‌ها
  را نمی‌دید) و مسیرِ «تشخیصِ ادمین‌شدن» (`onMyChatMember`) از قطعِ Postgres
  در همین سشن خراب شده بود. رفع: uploader/vpn/archive-bot هرکدام حالا موقعِ
  start و هر ۵ دقیقه از ads-bot می‌پرسند «به کمپینی وصل‌ام؟»
  (`ads.bot_status_check`، جایگزینِ کوئریِ Postgresِ حذف‌شده)، و وقتی وصل‌اند
  خودشان `membership.joined`/`left` را برای کانالِ خریدار منتشر می‌کنند
  (پکیجِ جدیدِ `shared/pkg/joinevents`، استخراج‌شده از منطقِ قبلیِ
  member-bot). رجوع CHANGELOG.md.
...
- (رفع‌شده ۲۰۲۶-۰۷-۱۰ با **dbmigrate**) migration ناهماهنگ بین سرویس‌ها:
  ...
  حالا سرویس `dbmigrate/` (رجوع README خودش) برای هر سرویس Postgres دار
  migration ورژن‌دار SQL دارد (baseline نسخه‌ی ۱ از AutoMigrate واقعی هر
  سرویس تولید شده) با دستورهای `up`/`status`/`mark`/`new`، جدول
  `schema_migrations` با checksum در هر دیتابیس، و ساخت خودکار دیتابیس. در
  ۲۰۲۶-۰۷-۱۰ این عدد ۱۱ سرویس بود؛ بعد از migration کاملِ vpn-bot/archive-bot/
  member-bot به MongoDB در ۲۰۲۶-۰۷-۱۷، الان **۸ سرویس** پوشش داده می‌شود
  (رجوع بخش ۲ و `dbmigrate/README.md`).
...
- بخش اجاره‌ی قفل کانال تازه به‌طور کامل طراحی شده؛ تست end-to-end با
  ربات واقعی و دیتابیس واقعی هنوز کامل انجام نشده. (۲۰۲۶-۰۷-۱۹) حلقه‌ی
  «تشخیصِ join واقعی → membership.joined» که تا این تاریخ اصلاً به هیچ‌جا
  وصل نبود، الان در کد سیم‌کشی شده (رجوع بخش ۷) — ولی هنوز با یک کاربرِ
  واقعی و کانالِ واقعی امتحان نشده، فقط build/vet/test تأیید شده.
...
- (رفع‌شده ۲۰۲۶-۰۷-۱۰، سخت‌ترشده ۲۰۲۶-۰۷-۲۰) **webhook-gateway `gateway.register` بدون auth**:
  هر کسی با دسترسی NATS می‌توانست webhook هر ربات دلخواهی را hijack کند. رفع: `SERVICE_HMAC_SECRET`
  به Config اضافه شد؛ ارسال‌کننده باید `service_id`+`service_key` معتبر داشته باشد (همان الگوی
  HMAC بقیه‌ی سرویس‌ها). **تصحیح ۲۰۲۶-۰۷-۲۰**: توضیحِ قبلیِ این‌جا («اگر secret خالی باشد، backward
  compatible») دیگر درست نیست — کد فعلی (`webhook-gateway/cmd/main.go`) با `log.Fatal` بالا
  نمی‌آید اگر `SERVICE_HMAC_SECRET` خالی باشد؛ یعنی الان همیشه الزامی است، نه اختیاری.
...
- (رفع‌شده ۲۰۲۶-۰۷-۲۰) **بازنویسیِ کاملِ `.env`/`.env.example` هر ۲۱ سرویس** با مقایسه‌ی
  مستقیمِ کد واقعی (نه فرض): **member-bot Lock HTTP API فعلاً fail-open بود**
  (`LOCK_API_SECRET` هیچ‌جا در `.env` نبود، و apiKey خالی یعنی درخواستِ بدونِ هدر هم رد
  نمی‌شد) — رفع شد. **`license-service/TEST_LICENSE_SECRET` یک secret واقعیِ کامیت‌شده در
  `.env.example` بود** و در `uploader-bot/.env.example` هم زیرِ نامِ اشتباه تکرار شده بود —
  rotate شد. `source-service`/`vpn-bot`/`archive-bot` هرکدام یک متغیرِ الزامیِ fail-closed
  (`SERVICE_HMAC_SECRET`/`LICENSE_TOKEN`) را اصلاً نداشتند — یعنی با `.env` قبلی‌شان اصلاً بالا
  نمی‌آمدند. `GRAFANA_PASSWORD` هم در `.env` ریشه نبود (پیش‌فرضِ ناامنِ `admin`). جزئیاتِ کاملِ
  هر سرویس در `CHANGELOG.md`.
