...
// TestServiceTypeLabel بررسی label های نوع سرویس.
func TestServiceTypeLabel(t *testing.T) {
	...
	for _, tt := range tests {
		label := serviceTypeLabel(tt.input)
		if !strings.Contains(label, tt.wantIcon) && tt.input != "unknown" {
			t.Errorf("serviceTypeLabel(%q) = %q, should contain %q", tt.input, label, tt.wantIcon)
		}
		if tt.input == "unknown" && label != "unknown" {
			t.Errorf("serviceTypeLabel(%q) = %q, want %q", tt.input, label, tt.input)
		}
	}
}
